-- Fix: "column reference \"business_name\" is ambiguous" during workspace setup.
--
-- Root cause: public.ensure_ledger_workspace declares a RETURNS TABLE column
-- named business_name. In PL/pgSQL that name becomes an output variable, so the
-- bare reference to business_name inside the INSERT ... RETURNING clause is
-- ambiguous (output variable vs. businesses.business_name column).
--
-- Fix: qualify the column with its table name in the RETURNING clause. Return
-- column names are kept unchanged (business_id, book_id, business_name) so the
-- client (row['business_name']) continues to work without changes.
...
begin
  ...
  if found_book_id is null then
    ...
    returning books.id into found_book_id;
  end if;
